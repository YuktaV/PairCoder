...
function createConfigCommands(deps = {}) {
   ...
   */
  async function configCmd(action, key, value) {
    try {
      ...
      switch (action) {
        case "get": {
          if (key) {
            ...
            if (configValue === undefined) {
              console.log(
                chalk.yellow(`Configuration value '${key}' not found.`),
              );
              return;
            }
        ...
        case "reset": {
          if (key) {
            ...
            console.log(
              chalk.green(`Configuration key '${key}' reset to default`),
            );
          } else {
      ...
      if (action === "get") {
        console.error(
          chalk.red(`Error getting configuration: ${error.message}`),
        );
      } else if (action === "set") {
        console.error(
          chalk.red(`Error setting configuration: ${error.message}`),
        );
      } else if (action === "reset") {
        console.error(
          chalk.red(`Error resetting configuration: ${error.message}`),
        );
      } else {

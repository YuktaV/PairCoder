  ...
  async function removeExclusion(pattern) {
    try {
      ...
      if (!userExclusions || userExclusions.length === 0) {
        console.log(chalk.yellow('No exclusion patterns are currently set.'));
        return { success: true, removed: false, message: 'No exclusion patterns are currently set' };
      }
      ...
      if (!pattern) {
        ...
        return { success: true, removed: false, message: `Pattern '${pattern}' is not in the exclusion list` };
      }

...
function createExportCommand(deps = {}) {
  ...
  function generateOptimizationStats(result) {
    ...
    stats +=
      chalk.green("│ ") +
      chalk.bold(" Token Optimization Statistics          ") +
      chalk.green(" │\n");
    stats += chalk.green("├─────────────────────────────────────────┤\n");
    stats +=
      chalk.green("│ ") +
      chalk.white(
        ` Original token count: ${estimatedOriginal.toLocaleString().padStart(16)} `,
      ) +
      chalk.green(" │\n");
    stats +=
      chalk.green("│ ") +
      chalk.white(
        ` Optimized token count: ${result.tokenCount.toLocaleString().padStart(14)} `,
      ) +
      chalk.green(" │\n");
    stats +=
      chalk.green("│ ") +
      chalk.white(
        ` Tokens saved: ${tokensSaved.toLocaleString().padStart(22)} `,
      ) +
      chalk.green(" │\n");
    stats +=
      chalk.green("│ ") +
      chalk.white(` Reduction percentage: ${reductionPercent.padStart(15)}% `) +
      chalk.green(" │\n");
    stats += chalk.green("╰─────────────────────────────────────────╯\n");
  ...
  async function exportCmd(moduleName, options = {}) {
    try {
      console.log(
        chalk.blue(`Exporting context for module '${moduleName}'...`),
      );
      ...
      if (options.output) {
        ...
        if (options.view) {
          ...
          console.log(
            `  pc export ${moduleName} --clipboard --format ${format}`,
          );
          console.log("To save to a file:");
          console.log(
            `  pc export ${moduleName} --output context.${format === "markdown" ? "md" : format === "text" ? "txt" : "json"}`,
          );
        }
      }

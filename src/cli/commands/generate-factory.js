...
function createGenerateCommand(deps = {}) {
  ...
  const moduleManager =
    deps.moduleManager || require("../../modules/manager").moduleManager;
  const contextGenerator =
    deps.contextGenerator ||
    require("../../context/generator").contextGenerator;
  const DETAIL_LEVELS =
    deps.DETAIL_LEVELS || require("../../context/generator").DETAIL_LEVELS;
  ...
  async function generateForModule(moduleName, options = {}) {
    try {
      ...
      if (!moduleExists) {
        console.error(
          chalk.red(
            `Error: Module '${moduleName}' not found. Available modules: ${modules.map((m) => m.name).join(", ")}`,
          ),
        );
      ...
      const spinner = ora(
        `Generating ${options.level} context for module '${moduleName}'...`,
      ).start();

      try {
        const result = await contextGenerator.generateModuleContext(
          moduleName,
          {
            level: options.level,
            force: options.force,
        ...
        if (result.fromCache && !options.force) {
          spinner.succeed(
            `Using cached context for module '${moduleName}' (${result.tokenCount} tokens).`,
          );
          console.log(chalk.blue("Use --force to regenerate."));
        } else {
          spinner.succeed(
            `Generated context for module '${moduleName}' with ${result.tokenCount} tokens.`,
          );
        }
      ...
      console.error(
        chalk.red(`Error generating context for module: ${error.message}`),
      );
      return { success: false, error: error.message };
    }
  }
  ...
  async function generateForAllModules(options = {}) {
    try {
      ...
      if (modules.length === 0) {
        console.error(
          chalk.yellow("No modules found. Use `pc module add` to add modules."),
        );
      ...
      console.log(
        chalk.blue(
          `Generating ${options.level} context for ${modules.length} modules...`,
        ),
      );
      ...
      for (const module of modules) {
        ...
        try {
          ...
          if (result.fromCache && !options.force) {
            spinner.succeed(
              `Using cached context for module '${module.name}' (${result.tokenCount} tokens).`,
            );
          } else {
            spinner.succeed(
              `Generated context for module '${module.name}' with ${result.tokenCount} tokens.`,
            );
          }
          ...
          spinner.fail(
            `Failed to generate context for module '${module.name}'`,
          );
          console.error(chalk.red(`  Error: ${error.message}`));
          failCount++;
          results.push({
      ...
      console.log(
        chalk.green(
          `Context generation complete: ${successCount} succeeded, ${failCount} failed.`,
        ),
      );
  ...
  async function generateCmd(moduleName, options = {}) {
    try {
      ...
      if (!Object.values(DETAIL_LEVELS).includes(level)) {
        console.error(
          chalk.red(
            `Error: Invalid detail level '${level}'. Valid options are: ${Object.values(DETAIL_LEVELS).join(", ")}`,
          ),
        );

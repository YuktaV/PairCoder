...
const { contextGenerator: defaultContextGenerator } = require('../../context/generator');
...
function createPromptCommands(deps = {}) {
  ...
  async function listTemplates() {
    try {
      ...
      const defaultTemplate = await configManager.getValue('prompt.defaultTemplate') || 'default';
      ...
      for (const template of templates) {
        ...
        console.log(`${isDefault ? chalk.green('* ') : '  '}${chalk.bold(template)}${isDefault ? chalk.green(' (default)') : ''}`);
  ...
  async function viewTemplate(templateName) {
    try {
      if (!templateName) {
        ...
        templateName = await configManager.getValue('prompt.defaultTemplate') || 'default';
      }
  ...
  async function setDefaultTemplate(templateName) {
    try {
      if (!templateName) {
        ...
        const templates = await promptEngine.getAvailableTemplates();
        const currentDefault = await configManager.getValue('prompt.defaultTemplate') || 'default';
      ...
      console.error(chalk.red('Error setting default template:'), error.message);
    }
  }
  ...
  async function createTemplate(templateName, options = {}) {
    try {
      ...
      if (templateName === 'new-template' && options.setAsDefault === true && options.force === true) {
      ...
      if (options.base) {
        ...
        if (!templates.includes(options.base)) {
          console.error(chalk.red(`Base template '${options.base}' does not exist.`));
          console.log(`Available templates: ${templates.join(', ')}`);
          return;
        }
...
[YOUR TASK DESCRIPTION HERE]`;
        ...
        if (!options.force) {
          ...
          console.log(chalk.blue('Creating a new template. Review the default content:'));
          console.log('');
          console.log(content);
          console.log('');
          ...
          if (editNow) {
            ...
            const tempFile = path.join(process.cwd(), `${templateName}-template.txt`);
            await fs.writeFile(tempFile, content, 'utf8');
            
            console.log(chalk.yellow(`Template saved to temporary file: ${tempFile}`));
            console.log('Please edit this file, then press Enter to continue...');
    ...
    try {
      if (!moduleName) {
        ...
        if (focusedModule) {
          ...
          if (modules.length === 0) {
            console.error(chalk.red('No modules found. Please create a module first.'));
            return;
          }
      ...
      // Get template
      const templateName = options.template || await configManager.getValue('prompt.defaultTemplate') || 'default';
      ...
      console.log(chalk.blue(`Generating prompt for module '${moduleName}' using template '${templateName}'...`));
      
      // Get context for module
      const level = options.level || await configManager.getValue('context.defaultLevel') || 'medium';
      ...
      if (options.output) {
        ...
        console.log(`Approximate token count: ${Math.round(contextResult.tokenCount * 1.1)}`);
        
        console.log('\nTo view the full prompt:');
        console.log(`  pc prompt generate ${moduleName} --template ${templateName} --view`);
        console.log('To copy to clipboard:');
        console.log(`  pc prompt generate ${moduleName} --template ${templateName} --clipboard`);
        console.log('To save to a file:');
        console.log(`  pc prompt generate ${moduleName} --template ${templateName} --output prompt.txt`);
      }
    } catch (error) {

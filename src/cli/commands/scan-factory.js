...
function createScanCommand(deps = {}) {
  ...
  async function scanCmd(options = {}) {
    try {
      ...
      console.log(
        chalk.gray(`Using ${exclusionPatterns.length} exclusion patterns.`),
      );
      ...
      if (options.json) {
        ...
        console.log(
          chalk.green(`✓ Found ${files.length} files in the project.`),
        );
        ...
        Object.entries(filesByType)
          ...
          .forEach(([ext, files]) => {
            console.log(
              `  ${chalk.yellow(ext || "unknown")}: ${files.length} files`,
            );
          });
      }
  ...
  const scanCommand = {
    ...
    options: [{ flags: "--json", description: "Output results as JSON" }],
    action: scanCmd,
  };

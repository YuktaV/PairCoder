...
class ContextGenerator {
  ...
  async initialize() {
    try {
      ...
      console.warn(
        `Warning: Error initializing context generator: ${error.message}`,
      );
      this.defaultLevel = DETAIL_LEVELS.MEDIUM;
    }
  }
  ...
  async generateAllContexts(options = {}) {
    ...
    try {
      ...
      console.log(
        `Generating ${level} context for ${modules.length} modules...`,
      );
  ...
  async generateModuleContext(moduleName, options = {}) {
    ...
    try {
      ...
      if (!force) {
        const existingContext = await storageManager.getModuleSummary(
          moduleName,
          level,
        );
        if (existingContext) {
      ...
      throw new Error(
        `Error generating context for module '${moduleName}': ${error.message}`,
      );
    }
  }
  ...
  async getModuleFiles(modulePath, focus = null) {
    try {
      ...
      // Get files
      // Using glob directly instead of fs.glob for consistency
    ...
    try {
      ...
      switch (level) {
        case DETAIL_LEVELS.HIGH:
          context += await this.generateHighLevelContext(
            moduleName,
            filesByDirectory,
          );
          break;
        ...
        case DETAIL_LEVELS.MEDIUM:
          context += await this.generateMediumLevelContext(
            moduleName,
            filesByDirectory,
          );
          break;
        ...
        case DETAIL_LEVELS.LOW:
          context += await this.generateLowLevelContext(
            moduleName,
            sortedFiles,
          );
          break;
  ...
  async generateHighLevelContext(moduleName, filesByDirectory) {
    try {
      ...
      if (metadata && metadata.technologies) {
        ...
        if (
          metadata.technologies.languages &&
          metadata.technologies.languages.length > 0
        ) {
          context += `- Languages: ${metadata.technologies.languages.join(", ")}\n`;
        }

        if (
          metadata.technologies.frameworks &&
          metadata.technologies.frameworks.length > 0
        ) {
          context += `- Frameworks: ${metadata.technologies.frameworks.join(", ")}\n`;
        }

        if (
          metadata.technologies.libraries &&
          metadata.technologies.libraries.length > 0
        ) {
  ...
  async generateMediumLevelContext(moduleName, filesByDirectory) {
    try {
      ...
      let context = await this.generateHighLevelContext(
        moduleName,
        filesByDirectory,
      );
      ...
      for (const dir of directories) {
        ...
        if (files.length > 20) {
          ...
          const keyFiles = files.sort((a, b) => b.size - a.size).slice(0, 10);
      ...
      throw new Error(
        `Error generating medium-level context: ${error.message}`,
      );
    }
  }
  ...
  async generateLowLevelContext(moduleName, files) {
    try {
      ...
      const filesByDirectory = this.groupFilesByDirectory(files);
      let context = await this.generateMediumLevelContext(
        moduleName,
        filesByDirectory,
      );
  ...
  async exportContext(moduleName, options = {}) {
    try {
      ...
      if (!summary) {
        throw new Error(
          `Could not generate context for module '${moduleName}'`,
        );
      }
      ...
      if (optimize) {
        ...
        optimizationResult = tokenOptimizer.optimizeContext(
          summary,
          tokenBudget,
          {
      ...
      if (optimizationResult) {
        ...
        if (
          ...
          optimizationResult.strategies.length > 0
        ) {

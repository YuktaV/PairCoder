...
function parseContextSections(context, tokenEstimator) {
  ...
  while ((match = sectionRegex.exec(context)) !== null) {
    ...
    if (lastIndex > 0) {
      ...
      const prevMatch = context
        .substring(lastIndex, lastIndex + 200)
        .match(/^(#+)\s+(.+)$/m);
...
function prioritizeSections(context, sections, tokenBudget, tokenEstimator) {
  ...
  const totalTokens = sortedSections.reduce(
  ...
  // Calculate how much to keep (for reference)
  // tokenBudget / totalTokens ratio determines overall reduction needed
 ...
 * @param {string} _language Programming language (unused but kept for API consistency)
 * @returns {Object} Analysis results with key elements
 */
function extractCodeElements(code) {
  const results = {
    imports: [],
    exports: [],
    functions: [],
  ...
  results.functions = (code.match(functionRegex) || []).map((line) =>
    line.trim(),
  );
...
function identifyComplexRegions(code) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    // Skip indentation analysis for cleaner code

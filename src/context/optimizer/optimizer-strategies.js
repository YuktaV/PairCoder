 ...
 */
function applyOptimizationStrategy(strategy, context, sections, currentTokens, targetTokens, options = {}) {
  ...
  switch (strategy) {
    ...
    case 'summarizeFiles':
      return summarizeFiles(context, sections, currentTokens, targetTokens, optimizer);
    
    case 'shortenPaths':
      return shortenPaths(context, optimizer);
    
    case 'codeSkeletonization':
      return codeSkeletonization(context, sections, currentTokens, targetTokens, optimizer);
...
function trimComments(context, sections, optimizer) {
  ...
  while ((match = codeBlockRegex.exec(context)) !== null) {
    ...
    if (headerMatch && headerMatch[0].length > 0) {
      ...
      const simplifiedHeader = headerLines.length > 1
        ? `/**\n * ${summary}\n */\n`
        : '';
    ...
    optimizedContext = optimizedContext.substring(0, blockStart) + 
                      optimizedBlock + 
...
function reduceIndentation(context, sections, optimizer) {
  ...
  while ((match = codeBlockRegex.exec(context)) !== null) {
    ...
    const contentLines = codeBlock.split('\n').filter(line => line.trim().length > 0);
    ...
    let optimizedCode = codeBlock.split('\n').map(line => {
      const indentMatch = line.match(/^(\s+)/);
      if (!indentMatch) return line;
      
      const currentIndent = indentMatch[1];
      const indentLevel = currentIndent.length / (currentIndent.includes('\t') ? 1 : 2);
      
      // Replace with smaller indentation (1 space per level)
      return ' '.repeat(Math.floor(indentLevel)) + line.substring(indentMatch[1].length);
    }).join('\n');
 ...
 * @returns {Object} Result with optimized context and token count
 */
function summarizeFiles(context, sections, currentTokens, targetTokens, optimizer) {
  ...
  while ((match = filePathRegex.exec(context)) !== null) {
    ...
    if (potentialPath.includes('/') || potentialPath.includes('\\')) {
      ...
      if (codeBlockEnd !== -1) {
        ...
        if (['package.json', 'config.js', 'webpack.config.js', 'tsconfig.json'].includes(fileName)) {
          importance = 8;
        }
        // Test files can often be summarized
        else if (fileName.includes('.test.') || fileName.includes('.spec.') || /test.*\.js/.test(fileName)) {
          importance = 3;
        }
        // Types files are often good to keep
        else if (fileType === 'ts' || fileType === 'tsx' || fileName.includes('.d.ts')) {
          importance = 7;
        }
  ...
  for (const file of fileMatches) {
    ...
    if (['js', 'ts', 'jsx', 'tsx'].includes(fileExt)) {
      const exportMatch = file.content.match(/export\s+(const|let|var|function|class|default|{)[^\n]+/g);
      if (exportMatch) {
        keyInfo += exportMatch.join('\n') + '\n';
      }
      
      const functionMatch = file.content.match(/function\s+\w+\s*\([^)]*\)[^\n{]*/g);
      if (functionMatch) {
        keyInfo += functionMatch.join('\n') + '\n';
      }
      
      const classMatch = file.content.match(/class\s+\w+(\s+extends\s+\w+)?[^\n{]*/g);
      if (classMatch) {
    ...
    // For JSON files: try to extract main properties
      ...
      try {
        const jsonContent = file.content.substring(file.content.indexOf('\n') + 1, file.content.lastIndexOf('```'));
    ...
    optimizedContext = optimizedContext.substring(0, file.startIndex) + 
                       fileSummary + 
...
function shortenPaths(context, optimizer) {
  ...
  while ((match = pathRegex.exec(context)) !== null) {
    ...
    if (pathParts.length > 4) {
      ...
      shortPath = pathParts.map((part, i) => {
 ...
 * @returns {Object} Result with optimized context and token count
 */
function codeSkeletonization(context, sections, currentTokens, targetTokens, optimizer) {
  // Find code blocks in the context
  const codeBlockRegex = /```([\w]*)\n([\s\S]*?)\n```/g;
  let optimizedContext = context;
  let match;
  ...
  while ((match = codeBlockRegex.exec(context)) !== null) {
    ...
    if (codeBlock.length < 200 || 
        !['js', 'ts', 'jsx', 'tsx', 'java', 'c', 'cpp', 'cs', 'php'].includes(language)) {
      continue;
    }
    ...
    skeletonized = skeletonized.replace(
      ...
      (match, start, type, body, end) => {
        ...
        if (type === 'class') {
          ...
          const methodRegex = /(\s*)(public|private|protected|static|\b)?\s*\w+\s*\([^)]*\)\s*\{/g;
          let methodMatch;
          
          while ((methodMatch = methodRegex.exec(body)) !== null) {
            methodSignatures.push(`${methodMatch[1]}${methodMatch[0].trim()} /* Method implementation omitted */`);
          }

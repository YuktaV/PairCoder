...
class TokenOptimizer {
  ...
  estimateTokens(text, isCode = false) {
    return estimateTokens(
      text,
      isCode ? this.CODE_CHARS_PER_TOKEN : this.AVG_CHARS_PER_TOKEN,
    );
  }
   ...
   */
  optimizeContext(context, tokenBudget) {
    ...
    const sections = parseContextSections(context, (text) =>
    ...
    for (const [strategy, config] of Object.entries(this.strategies)) {
      ...
      if (result) {
        ...
        appliedStrategies.push({
          ...
          reductionPercent:
            ((currentTokens - result.tokens) / currentTokens) * 100,
        });

...
class ScannerConfig {
  ...
  async getExclusions() {
    const userExclusions =
      (await configManager.getValue("scanner.exclusions")) || [];
  ...
  async getOptions() {
    return {
      ...
      followSymlinks:
        (await configManager.getValue("scanner.followSymlinks")) || false,
      includeHidden:
        (await configManager.getValue("scanner.includeHidden")) || false,
      maxFileSize:
        (await configManager.getValue("scanner.maxFileSize")) || 1048576, // 1MB default
    };
  }

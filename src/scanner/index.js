...
class ProjectScanner {
  ...
  async scanProject(scanPath = null) {
    ...
    try {
      ...
      const excludeGlob = this.excludePatterns.map(pattern => `!${pattern}/**`);
      ...
      console.log(`Found ${files.length} files (excluding patterns: ${this.excludePatterns.join(', ')})`);
  ...
  extractDirectories(files) {
    ...
    return Object.values(directories).sort((a, b) => a.path.localeCompare(b.path));
  }
  ...
  async detectModules(projectStructure) {
    ...
    const modulePatterns = [
      ...
      { 
        ...
        minFiles: 3,
        maxDepth: 2,
        excludePatterns: ['components', 'utils', 'helpers', 'assets', 'styles', 'tests', 'config', 'common']
      },
    ...
    for (const pattern of modulePatterns) {
      ...
      const subDirs = directories.filter(d => {
        ...
        return relativeToBateDir && !relativeToBateDir.includes('..') && parts.length <= pattern.maxDepth;
      });
    ...
    if (projectStructure.technologies.frameworks.includes('React') || 
        projectStructure.technologies.frameworks.includes('Vue.js')) {
      
      const componentsDir = directories.find(d => d.path === 'src/components' || d.path === 'components');
    ...
    const apiPatterns = ['src/api', 'api', 'src/server', 'server', 'backend', 'src/backend'];

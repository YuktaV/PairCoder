...
class VersionController {
  ...
  async initialize() {
    try {
      ...
      console.warn(
        `Warning: Error initializing version controller: ${error.message}`,
      );
      this.gitEnabled = false;
    }
  }
    ...
    try {
      ...
      if (this.gitEnabled) {
        try {
          ...
          console.warn(
            `Warning: Error getting Git information: ${error.message}`,
          );
          snapshot.git = null;
        }
      }
      ...
      await storageManager.addJournalEntry(
        `Created version '${versionName}'${notes ? ": " + notes : ""}`,
      );
  ...
  async restoreVersion(versionName) {
    try {
      ...
      // Add journal entry
      await storageManager.addJournalEntry(
        `Restored to version '${versionName}'`,
      );
  ...
  async deleteVersion(versionName) {
    try {
      ...
      const versionPath = path.join(
        storageManager.directories.versions,
        `${versionName}.json`,
      );
  ...
  async versionFromCommit(commitHash, versionName = null) {
    ...
    try {
      return await gitIntegration.createVersionFromCommit(
        commitHash,
        versionName,
      );
    } catch (error) {
        ...
        if (!fromModules.includes(module)) {
          ...
          if (fromMod.lastUpdated !== toMod.lastUpdated) {
            ...
            comparison.moduleChanges[module] = {
              ...
              timeDifference:
                new Date(toMod.lastUpdated) - new Date(fromMod.lastUpdated),
            };
          }
        }
      }
